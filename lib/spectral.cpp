...
int Spectral::read_data(const char* data){

	// Read data
	cout << "cpp: reading " << data << endl;
	unsigned int row = 0;
	const char sep = ',';
	ifstream file(data);
	if(file.is_open()){
		...
		file.close();
		cout << "cpp: reading finished" << endl;
		return(1);
	}else{
		return(0);
	}
...
int Spectral::write_data(const char* data){

	// Read data
	cout << "cpp: writing " << data << endl;
	unsigned int row = 0;
	const char sep = ',';
	ofstream file(data);
	if(file.is_open()){
		...
		file.close();
		cout << "cpp: writing finished" << endl;
		return(1);
	}else{
		return(0);
	}
...
void Spectral::cluster(){
	...
	cout << "cpp: generating kernel matrix..." << endl;
	generate_kernel_matrix();
	cout << "cpp: eigendecomposition..." << endl;
	eigendecomposition();
	cout << "cpp: kmeans..." << endl;
	kmeans();
	cout << "cpp: all done." << endl;
...
void Spectral::kmeans(){
	...
	for (unsigned int n=0; n < max_iters; n++){
		...
		printf("cpp: Iterations %i : Error %2.4f : Error delta %2.4f : Centroid movement %2.4f\n",n+1,e,ediff,cdiff);
